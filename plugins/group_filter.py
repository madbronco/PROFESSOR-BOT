...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("ഇത് താങ്കളുടെ സെർച്ച് റിസൾട്ട് അല്ല. ഫയൽ ലഭിക്കാനായി താങ്കൾ തന്നെ സെർച്ച് ചെയ്യുക.", show_alert=True)
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_name}", callback_data=f'{pre}#{nxreq}#{file.file_id}')] for file in files ]
        else:
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('Checking File In Database')
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('This File Is Not Found In Database')
